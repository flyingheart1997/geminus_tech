...
export const Companies = () => {
    ...
    return (
        <section className='flex h-auto w-full relative overflow-hidden bg-[#060D0D] shadow-sm'>
            ...
            <ContainerProvider>
                <div className='grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 z-10 py-16 gap-10 items-start justify-center' >
                    {CompaniesList.map((company, index) => (
                        <motion.div
                            ...
                            key={company.src} className='shrink-0 z-10 flex items-center justify-center bg-[rgba(20,29,26,0.52)] h-[60px] px-[36px] badge__shadow rounded-full '>
                            <Image src={company.src} alt={company.src} width={100} height={100} objectFit='contain' className='object-contain h-[24px]' />
                        </motion.div>
                    ))}
                </div>
            </ContainerProvider>
