                ...
                {contents.map((content, index) => (
                    <div key={index} className={cn('gap-10 grid grid-cols-1 lg:grid-cols-2 items-center', index % 2 === 1 ? 'lg:reverse' : '')}>
                        <div className='flex flex-col gap-4'>
                            {content.title &&
                                <span className='text-[clamp(16px,2.5vw,20px)] text-black font-semibold text-wrap leading-6'>
                                    {content.title}
                                </span>
                            }
                        ...
                        <div className='flex items-center justify-center relative rounded-md'>
                            <Image src={content.image ? content.image : '/overview.svg'} alt={content.title || ''} width={100} height={100} className='object-cover w-full h-full rounded-md' />
                        </div>
                    </div>
                ))}
            </div>
